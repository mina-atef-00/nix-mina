...

{
  ...
  services = {
    ...
    greetd = {
      ...
      settings = {
        default_session = {
          command = "${pkgs.greetd.tuigreet}/bin/tuigreet --time --time-format '%A, %d %B %Y - %I:%M %p' --greeting 'Hello mina' --greet-align center --remember --remember-user-session --user-menu --cmd mango";
          user = "greeter";
        };
      };
    };
